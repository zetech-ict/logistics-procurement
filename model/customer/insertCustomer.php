<?php
	...
	if(isset($_POST['customerDetailsCustomerFullName'])){
		...
		$county = htmlentities($_POST['customerDetailsCustomerCity']);
		$constituency = htmlentities($_POST['customerDetailsCustomerDistrict']);
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO customer(fullName, email, mobile, phone2, address, address2, county, constituency, status) VALUES(:fullName, :email, :mobile, :phone2, :address, :address2, :city, :district, :status)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'phone2' => $phone2, 'address' => $address, 'address2' => $address2, 'city' => $county, 'district' => $constituency, 'status' => $status]);
