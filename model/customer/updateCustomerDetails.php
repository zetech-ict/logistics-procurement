<?php
	...
	if(isset($_POST['customerDetailsCustomerID'])) {
		...
		$customerDetailsCustomerCounty = htmlentities($_POST['customerDetailsCustomerCity']);
		$customerDetailsCustomerConstituency = htmlentities($_POST['customerDetailsCustomerDistrict']);
		...
		if(isset($customerDetailsCustomerFullName) && isset($customerDetailsCustomerMobile) && isset($customerDetailsCustomerAddress)) {
			...
			if($customerIDSelectStatement->rowCount() > 0) {
				...
				$updateCustomerDetailsSql = 'UPDATE customer SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, address2 = :address2, county = :city, constituency = :district, status = :status WHERE customerID = :customerID';
				$updateCustomerDetailsStatement = $conn->prepare($updateCustomerDetailsSql);
				$updateCustomerDetailsStatement->execute(['fullName' => $customerDetailsCustomerFullName, 'email' => $customerDetailsCustomerEmail, 'mobile' => $customerDetailsCustomerMobile, 'phone2' => $customerDetailsCustomerPhone2, 'address' => $customerDetailsCustomerAddress, 'address2' => $customerDetailsCustomerAddress2, 'city' => $customerDetailsCustomerCounty, 'district' => $customerDetailsCustomerConstituency, 'status' => $customerDetailsStatus, 'customerID' => $customerDetailsCustomerID]);
