<?php
	...
	if(isset($_POST['vendorDetailsVendorID'])) {
		...
		$vendorDetailsVendorCounty = htmlentities($_POST['vendorDetailsVendorCity']);
		$vendorDetailsVendorConstituency = htmlentities($_POST['vendorDetailsVendorDistrict']);
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				if($vendorIDSelectStatement->rowCount() > 0) {
					...
					$updateVendorDetailsSql = 'UPDATE vendor SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, address2 = :address2, county = :city, constituency = :district, status = :status WHERE vendorID = :vendorID';
					$updateVendorDetailsStatement = $conn->prepare($updateVendorDetailsSql);
					$updateVendorDetailsStatement->execute(['fullName' => $vendorDetailsVendorFullName, 'email' => $vendorDetailsVendorEmail, 'mobile' => $vendorDetailsVendorMobile, 'phone2' => $vendorDetailsVendorPhone2, 'address' => $vendorDetailsVendorAddress, 'address2' => $vendorDetailsVendorAddress2, 'city' => $vendorDetailsVendorCounty, 'district' => $vendorDetailsVendorConstituency, 'vendorID' => $vendorDetailsVendorID, 'status' => $vendorDetailsStatus]);
